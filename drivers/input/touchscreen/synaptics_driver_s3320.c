...
prEntry_tmp = proc_create("double_swipe_enable", 0666, prEntry_tp, &double_swipe_enable_proc_fops);
	...
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
		printk(KERN_INFO"init_synaptics_proc: Couldn't create right_arrow_enable\n");
	}

    prEntry_tmp = proc_create("up_arrow_enable", 0666, prEntry_tp, &up_arrow_enable_proc_fops);
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
		printk(KERN_INFO"init_synaptics_proc: Couldn't create up_arrow_enable\n");
	}

    prEntry_tmp = proc_create("down_arrow_enable", 0666, prEntry_tp, &down_arrow_enable_proc_fops);
	if(prEntry_tmp == NULL){
		ret = -ENOMEM;
		printk(KERN_INFO"init_synaptics_proc: Couldn't create down_arrow_enable\n");
	}
#endif
