...
//add by jiachenghui for otg switch, 2015-5-6
#ifdef VENDOR_EDIT
...
#endif
//end add by jiachenghui for otg switch, 2015-5-6
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_UNDEFINED:
		...
		//add by jiachenghui for otg switch, 2015-5-8
		#ifdef VENDOR_EDIT
		if (!oem_test_id(ID, &dotg->inputs)) {
		#else
		//end add by jiachenghui for otg switch, 2015-5-8
		if (!test_bit(ID, &dotg->inputs)) {
		#endif//add by jiachenghui for otg switch, 2015-5-8
			dev_dbg(phy->dev, "!id\n");
